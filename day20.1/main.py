...
def get_section_step_length(section):
    assert section[0] not in  ['(', ')']
    results = [0]
    parens_stack = []
    for idx in range(len(section)):
        ...
        if step in ['E', 'N', 'W', 'S']:
            ...
            if len(parens_stack) == 0:
                results[-1] += max(get_section_step_length(section[starting+1:idx]))            
        elif step == '|':
            if len(parens_stack) == 0:
                results.append(0)
...
def sanitize(directions):
    while True:
        next_directions = directions.replace('WE', '').replace('EW', '').replace('SN', '').replace('NS', '')
        if len(next_directions) == len(directions):
...
def main():
    ...
    print get_section_step_length(sanitize(directions))
