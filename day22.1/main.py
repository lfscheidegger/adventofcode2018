...
def get_total_risk():
    ...
    erosion_levels = list(list(0 for x in range(TARGET[0] + 1)) for y in range(TARGET[1] + 1))
    ...
    erosion_levels[TARGET[1]][TARGET[0]] = 0

    print_cave(erosion_levels)
            
    total_risk_level = 0
    for y in range(TARGET[1] + 1):
        for x in range(TARGET[0] + 1):
            total_risk_level += erosion_levels[y][x] % 3
